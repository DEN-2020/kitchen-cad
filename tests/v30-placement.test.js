...
test('left-wall snap keeps a 1200x600 rotated module inside room',()=>{
 const pose=wallSnapPose({roomWidth:3000,roomDepth:2500,moduleWidth:1200,moduleDepth:600,centerX:250,centerZ:1250,threshold:300,grid:50});
 assert.equal(pose?.rotationY,90);
 assertInside(pose,1200,600);
});

test('right-wall snap keeps a 1200x600 rotated module inside room',()=>{
 const pose=wallSnapPose({roomWidth:3000,roomDepth:2500,moduleWidth:1200,moduleDepth:600,centerX:2750,centerZ:1250,threshold:300,grid:50});
