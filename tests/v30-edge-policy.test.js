...
test('carcass edge policy bands visible front edges only',()=>{
 const p=createProject(),m=createModule('base');m.width=600;m.height=720;m.depth=560;m.shelfCount=1;m.doorCount=1;p.modules=[m];p.fixtures=[];p.countertop.enabled=false;
...
test('600 cabinet edge total is plausible and not all carcass sides',()=>{
 const p=createProject(),m=createModule('base');m.width=600;m.height=720;m.depth=560;m.shelfCount=1;m.doorCount=1;p.modules=[m];p.fixtures=[];p.countertop.enabled=false;
 const model=buildProject(p);const total=model.parts.reduce((s,x)=>s+edgeBandMeters(x),0);
 assert.ok(total>5&&total<7,`edge total ${total} m`);
