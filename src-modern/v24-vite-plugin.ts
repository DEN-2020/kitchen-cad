 ...
 return {
  ...
  transform(code:string,id:string){
   if(id.endsWith('/src-modern/domain/core.ts')){
    ...
    next=next.replace(" const p=normalizeEditorProject(project),model:any=buildProject(p),top=p.countertop,ct=model.objects.find((o:any)=>o.id==='CT-01');if(ct){ct.size=[top.length,top.thickness,top.depth];ct.center=[top.offsetX+top.length/2,top.elevation+top.thickness/2,top.offsetZ+top.depth/2];model.countertop={...(model.countertop||{}),id:'CT-01',length:top.length,depth:top.depth,thickness:top.thickness,x:top.offsetX,z:top.offsetZ,elevation:top.elevation}}", " const p=normalizeEditorProject(project),model:any=buildProject(p),top=p.countertop;model.objects=model.objects.filter((o:any)=>o.id!=='CT-01');const segments=buildCountertopSegments(p,model.modules);model.countertopSegments=segments;model.countertop=segments[0]||null;for(const s of segments)model.objects.push({...s,kind:'countertop-segment',role:'countertop',moduleId:null,appearance:{pattern:'stone',color:top.color||'#e7e5dd',gloss:!!top.gloss}});for(const f of model.fixtures||[]){const seg=countertopSegmentForModule(segments,f.targetModuleId);const obj=model.objects.find((o:any)=>o.id===f.id);if(seg&&obj){obj.center=[obj.center[0],seg.elevation+seg.thickness+3,obj.center[2]];f.y=obj.center[1];f.countertopSegmentId=seg.id}}");
