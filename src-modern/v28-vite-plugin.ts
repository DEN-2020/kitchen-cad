 ...
 return {
  ...
  transform(code:string,id:string){
   ...
   if(id.endsWith('/src-modern/domain/core.ts')){
    let next=code;
    const ns=next.indexOf('export function normalizeEditorProject'),ne=next.indexOf('export function deriveModel',ns);if(ns<0||ne<0)throw new Error('v28 normalize block not found');
    const block=next.slice(ns,ne),marker='return p}',rel=block.lastIndexOf(marker);if(rel<0)throw new Error('v28 normalize return not found');
    const normalizeInject="for(const m of p.modules||[]){if(['cornerBaseDiagonal','cornerBaseL'].includes(m.type)){if(!Number.isFinite(m.cornerRunDepth)||m.cornerRunDepth<=0)m.cornerRunDepth=600;if(!Number.isFinite(m.cornerWingDepth)||m.cornerWingDepth===560)m.cornerWingDepth=m.cornerRunDepth}else if(['cornerWallDiagonal','cornerWallL'].includes(m.type)){if(!Number.isFinite(m.cornerRunDepth)||m.cornerRunDepth<=0)m.cornerRunDepth=320;if(!Number.isFinite(m.cornerWingDepth))m.cornerWingDepth=m.cornerRunDepth}if(m.type==='cornerBaseDiagonal'&&m.width!==m.depth){const side=Math.max(m.width,m.depth,m.cornerRunDepth+150);m.width=side;m.depth=side}}return p}";
    const abs=ns+rel;next=next.slice(0,abs)+normalizeInject+next.slice(abs+marker.length);
    next=next.replace(/export function updateModule\(project:any,id:string,patch:Record<string,unknown>\)\{[\s\S]*?return p\}/,"export function updateModule(project:any,id:string,patch:Record<string,unknown>){const p=clone(project),m=p.modules.find((x:any)=>x.id===id);if(m){Object.assign(m,patch);if(['cornerBaseDiagonal','cornerBaseL'].includes(m.type)&&'cornerRunDepth'in patch){m.cornerWingDepth=Number(m.cornerRunDepth)||600}if(m.type==='cornerBaseDiagonal'&&('width'in patch||'depth'in patch)){const side='width'in patch?Number(m.width):Number(m.depth);if(Number.isFinite(side)){m.width=side;m.depth=side}}}return p}");
    if(!next.includes("m.type==='cornerBaseDiagonal'&&m.width!==m.depth")||!next.includes("'cornerRunDepth'in patch"))throw new Error('v28 domain corner semantics transform did not apply');
    return next;
   }
