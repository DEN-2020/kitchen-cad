...
function materialKey(part){return [part.substrate||'unspecified',part.decor||'unspecified',part.appearance?.color||'unspecified',n(part.thickness,0)].join('|')}
function sheetsFor(parts,price,c){
 const sheetArea=c.sheetWidth*c.sheetHeight/1e6,usable=sheetArea*Math.max(.25,1-c.wastePercent/100),groups=new Map();
 for(const part of parts){const key=materialKey(part),current=groups.get(key)||{substrate:part.substrate||'unspecified',decor:part.decor||'unspecified',color:part.appearance?.color||'',thickness:n(part.thickness,0),area:0};current.area+=areaM2(part);groups.set(key,current)}
 const batches=[...groups.values()].map(batch=>{const sheets=batch.area>0?Math.ceil(batch.area/usable):0;return{...batch,sheets,cost:sheets*price}}),area=batches.reduce((sum,batch)=>sum+batch.area,0),sheets=batches.reduce((sum,batch)=>sum+batch.sheets,0),cost=batches.reduce((sum,batch)=>sum+batch.cost,0);
 return{area,sheetArea,usable,sheets,cost,batches}
}
