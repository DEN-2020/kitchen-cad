...
export function ensureProjectDefaults(p){
  ...
  p.defaults={...DEFAULT_MODULE_STYLE,...(p.defaults||{})};if(!MATERIAL_PRODUCTS[p.defaults.bodyMaterialId]?.roles.includes('body'))p.defaults.bodyMaterialId=inferMaterialProductId({role:'body',substrate:p.defaults.bodySubstrate,gloss:false});if(!MATERIAL_PRODUCTS[p.defaults.frontMaterialId]?.roles.includes('front'))p.defaults.frontMaterialId=inferMaterialProductId({role:'front',substrate:p.defaults.frontSubstrate,gloss:p.defaults.gloss});Object.assign(p.defaults,materialSelectionPatch('body',p.defaults.bodyMaterialId),materialSelectionPatch('front',p.defaults.frontMaterialId)); if(!Number.isFinite(p.defaults.washerClearance))p.defaults.washerClearance=10; p.defaults.washerClearance=Math.max(5,Math.min(60,p.defaults.washerClearance));
