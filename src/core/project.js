...
export function createModule(type='base') {
  ...
  return {
    ...
    cornerOpening:d.cornerOpening ?? 0, cornerWingDepth:d.cornerWingDepth ?? 0, cornerRunDepth:d.cornerRunDepth ?? 0,
...
export function ensureProjectDefaults(p){
  ...
  for(const m of p.modules||[]){
    const d=typeDefaults[m.type]||typeDefaults.base; if(!Number.isFinite(m.offsetX))m.offsetX=0;if(!Number.isFinite(m.offsetZ))m.offsetZ=0;if(!Number.isFinite(m.elevation))m.elevation=d.elevation??0;if(!Number.isFinite(m.feet))m.feet=d.feet??0;if(!Number.isFinite(m.cornerOpening))m.cornerOpening=d.cornerOpening??0;if(!Number.isFinite(m.cornerWingDepth))m.cornerWingDepth=d.cornerWingDepth??0;if(!Number.isFinite(m.cornerRunDepth))m.cornerRunDepth=d.cornerRunDepth??0;if(['cornerBaseDiagonal','cornerBaseL'].includes(m.type)&&m.cornerRunDepth===0)m.cornerRunDepth=600;if(['cornerWallDiagonal','cornerWallL'].includes(m.type)&&m.cornerRunDepth===0)m.cornerRunDepth=320;
...
export function validateProject(raw){
 ...
 for(const m of p.modules){if(!m||typeof m!=='object'||typeof m.id!=='string'||ids.has(m.id))throw new Error('Повторяющийся или некорректный ID модуля');ids.add(m.id);member(m.type,Object.keys(MODULE_TYPES),'тип модуля');number(m.width,100,3000,'Ширина');number(m.height,100,3000,'Высота');number(m.depth,20,1500,'Глубина');number(m.offsetX,-10000,10000,'Смещение X');number(m.offsetZ,-10000,10000,'Смещение Z');number(m.elevation,0,3000,'Отметка низа');number(m.feet,0,300,'Ножки');
   if(m.cornerOpening)number(m.cornerOpening,150,1000,'Проём углового модуля');if(m.cornerWingDepth)number(m.cornerWingDepth,150,1200,'Глубина крыла углового модуля');if(m.cornerRunDepth)number(m.cornerRunDepth,150,1200,'Глубина ряда углового модуля');
